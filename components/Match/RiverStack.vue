<template>
    <div class="flex flex-col">
        ...
        <div v-for="(card, index) in cards" v-if="cards.length > 0" @click="$emit('clicked')"
            :class="`first:z-0 z-10 last:z-20  border-t-2 border-x-2 last:border-b-2 border-slate-300 -my-12 w-24 h-32 bg-slate-200 flex flex-col justify-between ${card ? 'cursor-pointer' : ''} select-none`"
            @dragover="dragoverHandler($event)" @drop="dropHandler($event)" @dragstart="dragStartHandler($event, index)"
